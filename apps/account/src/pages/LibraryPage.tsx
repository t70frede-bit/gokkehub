...
import { Button, GameCover, Modal, Toggle, useToast } from "@gokkehub/ui";
...
function GameCard({
  ...
  return (
    ...
    >
      ...
      >
        <GameCover steamAppId={game.steam_app_id} name={game.display_name} />
        ...
        >
          {results.map((item) => {
            ...
            return (
              ...
              >
                <div className="flex-shrink-0 rounded-sm overflow-hidden" style={{ width: 32, height: 48 }}>
                  <GameCover steamAppId={item.appid} name={item.name} />
                </div>
    ...
    <Modal open onClose={onClose}>
      <div className="flex flex-col gap-4" style={{ minWidth: "min(80vw, 420px)" }}>
        <div className="flex gap-4 items-start">
          <div className="rounded-sm overflow-hidden flex-shrink-0" style={{ width: 72, aspectRatio: "2/3" }}>
            <GameCover steamAppId={game.steam_app_id} name={game.display_name} />
          </div>
...
export default function LibraryPage({ session }: Props) {
  ...
  const loading = gamesLoading || challengesLoading;
  ...
  async function handleSyncSteam() {
    ...
    setImporting(true);
    try {
      const res = await fetch(`/steam/games?input=${encodeURIComponent(steamId)}`);
      ...
      const count = await bulkImport(
        data.games.map((g) => ({ name: g.name, steamAppId: g.appid })),
      );
      if (count === -1) {
        ...
        addToast(
          count > 0
            ? `Imported ${count} new game${count !== 1 ? "s" : ""} from Steam.`
            : `Library up to date — all ${data.games.length} Steam game${data.games.length !== 1 ? "s" : ""} already synced.`,
          "success",
        );
      }
    } catch {
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          {session.steamId && (
            <button
              ...
              disabled={importing}
              className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg transition-all disabled:opacity-60 flex-shrink-0"
              style={{
            ...
            >
              {importing ? "Importing…" : "⬇ Sync full library"}
            </button>
          )}
        </div>
        ...
        {!session.steamId && (
          <p className="text-xs" style={{ color: "rgb(var(--text-muted-rgb))" }}>
            Set your Steam ID on the <a href="/profile" style={{ color: "rgba(26,159,255,0.8)" }}>Profile</a> page to enable bulk import.
          </p>
        )}
      </div>
