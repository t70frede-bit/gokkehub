...
import type { PlayerChallenge, ChallengeType } from "../lib/types";

export function usePlayerChallenges(session: PublicSessionData | null) {
  ...
  async function addChallenge(text: string, type: ChallengeType, gameKey: string) {
    if (!session) return;
